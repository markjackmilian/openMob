...
- `src/openMob.Core/ViewModels/ChatViewModel.cs` — add `QuestionRequestedEvent` SSE handler, `AnswerQuestionAsync` command, `HasPendingQuestions` property, `_pendingQuestionCount` counter, `_inFlightQuestionAnswers` set, recovery call in `LoadMessagesAsync`, `ResetQuestionState` helper, `SetSession` reset call
...
- Wire format for the `question` SSE event is assumed as:
  ...
  - `allowFreeText` defaults to `true` if absent (Open Question 4)
  - The `POST /tui/control/response` body is `{ "body": "<answer>" }` (note: field name is `"body"`, not `"answer"`)

### Technical Risks

- **Wire format uncertainty (High):** Open Questions 1 and 2 are unresolved. The SSE event type string and JSON field names are assumed. The parser must be written defensively and updated once confirmed via live server inspection. The `[DEBUG]` log in `HandleUnknownEvent` already prints `e.RawType` and `e.RawData` — triggering a question from the server will reveal the exact format.
- **Long-poll behaviour of `GET /tui/control/next` (Medium):** If the endpoint blocks until a control request is available, `LoadMessagesAsync` would hang indefinitely without the 2-second timeout CTS. The timeout guard is mandatory.
- **`ChatMessage` constructor growth:** The constructor already has 18 parameters. Adding 6 more question-specific parameters increases it to 24. This is acceptable given the existing pattern, but the constructor must be updated carefully to preserve all existing callers (all use named parameters, so adding new optional parameters at the end is safe).
- **`SetSession` must reset question state:** `SetSession` already calls `ResetPermissionState()`. A parallel `ResetQuestionState()` call must be added to clear `_pendingQuestionCount`, `HasPendingQuestions`, and `_inFlightQuestionAnswers` when switching sessions.
- **XAML binding for `AnswerQuestionCommand`:** The command takes two parameters (`questionId` and `answer`). In XAML, the option chip buttons bind `CommandParameter` to the option string, but the `questionId` must come from the `ChatMessage`. The command signature must accept a tuple or a dedicated parameter object. **Resolution:** Use a `CommandParameter` that is a `Tuple<string, string>` or, more practically, bind the command to the `ChatViewModel` via the page's `BindingContext` and pass a `string[]` or use a `MultiBinding` workaround. The cleanest MAUI-compatible approach is to expose a single-parameter command `AnswerQuestionCommand(string[] args)` where `args[0]` = questionId and `args[1]` = answer, or to use a `ValueTuple`. **Preferred approach:** Expose `AnswerQuestionCommand` as `[RelayCommand]` accepting a `(string questionId, string answer)` tuple — MAUI supports `CommandParameter` as object, so the UI layer can pass a `string[]` or use a converter. The `om-mobile-ui` agent must implement the binding using a `MultiValueConverter` or a code-behind helper if MAUI's compiled bindings do not support multi-parameter commands natively. **Alternative (simpler):** Expose a `SelectedQuestionId` property on `ChatMessage` and bind the command to a single-string answer, with the `questionId` read from the message's `QuestionId` property inside the command. This is the cleanest approach and mirrors how `ReplyToPermissionAlwaysAsync` works (it takes only `requestId`, not the full permission object). **Decision:** `AnswerQuestionAsync` takes `(string questionId, string answer)` as a tuple parameter `(string, string)`. In XAML, use a `MultiBinding` with a converter, or pass a `string[]` via `CommandParameter`. The `om-mobile-ui` agent will choose the most idiomatic MAUI approach.

### Execution Order

> Steps that can run in parallel are marked with ⟳. Steps that must be sequential are numbered.

1. **[Git Flow]** Create branch `feature/question-tui-control` from `develop`
2. **[om-mobile-core]** Implement all Core changes: enums, records, DTO, parser extension, `ChatMessage` factory, API client methods, ViewModel handler + command + recovery
3. ⟳ **[om-mobile-ui]** Implement `QuestionRequest` DataTemplate in `ChatPage.xaml` — can start once the ViewModel binding surface is defined (after step 2 is complete enough to know property names)
4. **[om-tester]** Write unit tests for `ChatViewModel` question handling — must run after step 2
5. **[om-reviewer]** Full review against spec — must run after steps 2, 3, and 4
...
- [ ] Unit tests written for all new ViewModel logic (SSE handler, command, recovery, duplicate guard, counter)
